...
// Function to load cached coins
function loadCachedCoins() {
    const cachedCoins = localStorage.getItem('cachedCoins');
    if (cachedCoins) {
        coinCache = JSON.parse(cachedCoins);
    }
}

// Function to save cached coins
function saveCachedCoins() {
    localStorage.setItem('cachedCoins', JSON.stringify(coinCache));
}
...
// Function to fetch coins from API with retry mechanism and caching
function fetchCoinsWithRetry(retryCount = 0) {
    console.log("Fetching coins...");
    $('#loadingSpinner').show();

    // Check if we have cached data and it's less than 1 hour old
    const cachedCoinsData = localStorage.getItem('coinsData');
    const cachedTimestamp = localStorage.getItem('coinsDataTimestamp');
    
    if (cachedCoinsData && cachedTimestamp && (Date.now() - parseInt(cachedTimestamp) < 3600000)) {
        console.log("Using cached coins data");
        displayCoins(JSON.parse(cachedCoinsData));
        $('#loadingSpinner').hide();
        return;
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            localStorage.setItem('coinsDataTimestamp', Date.now().toString());
            displayCoins(data);
            $('#loadingSpinner').hide();
        },
        error: function(error) {
            if (error.status === 429 && retryCount < 3) {
                console.log(`Rate limit hit. Retrying in ${(retryCount + 1) * 5} seconds...`);
                setTimeout(() => fetchCoinsWithRetry(retryCount + 1), (retryCount + 1) * 5000);
            } else {
    ...
    $searchInput.on('input', function() {
        ...
        if (searchTerm.length > 0) {
            ...
            if (matchingCoins && matchingCoins.length > 0) {
                ...
                $searchResults.append('<span class="dropdown-item no-results">No matches found</span>');
            }
            $searchResults.show();
        } else {
            $searchResults.hide();
...
function filterCoins(searchTerm) {
    ...
    if ($('#liveReportChart').length) {
        ...
        $('#selectedCoinsList > div').each(function() {
            ...
            $(this).toggle(coinData.includes(searchTerm));
        });
    } else {
        // On home page
        $('.coin-card').each(function() {
            const coinSymbol = $(this).find('.card-title').text().toLowerCase();
            $(this).toggle(coinSymbol.includes(searchTerm));
        });
    }
}
...
function loadReportsPage() {
    ...
    console.log("Selected coins:", selectedCoins);  // Debug log
    pageTransition(() => {
...
function displaySelectedCoins() {
    console.log("Displaying selected coins:", selectedCoins);
    const coinsList = $('#selectedCoinsList');
    coinsList.empty();
    
    if (selectedCoins.length === 0) {
        coinsList.append('<p>No coins selected. Please go to the home page to select coins.</p>');
        return;
    }
    ...
    selectedCoins.forEach(coin => {
        ...
        coinsList.append(`
            <div class="col-sm-6 col-md-4 col-lg-3 mb-4 coin-card" data-coin="${coin.id}">
                <div class="card h-100">
                    <div class="card-body d-flex flex-column">
                        <img src="${coinInfo.image || 'images/coin-placeholder.png'}" alt="${coin.symbol}" class="coin-logo mb-2">
                        <h5 class="card-title">${coin.symbol.toUpperCase()} - ${coin.id}</h5>
                        <p>USD: $${coinInfo.usdPrice || 'Loading...'}</p>
                        <p>EUR: €${coinInfo.eurPrice || 'Loading...'}</p>
                        <p>ILS: ₪${coinInfo.ilsPrice || 'Loading...'}</p>
                        <button class="btn btn-danger btn-sm remove-from-report mt-auto" data-id="${coin.id}" data-symbol="${coin.symbol}">Remove</button>
                    </div>
                </div>
            </div>
        `);
    ...
    // Add button to add more coins if less than 5 coins are selected
    if (selectedCoins.length < 5) {
        coinsList.append(`
            <div class="col-sm-6 col-md-4 col-lg-3 mb-4">
                <div class="card h-100 add-coin-card">
...
function initBitcoinChart() {
    ...
    // Check for cached Bitcoin data
    const cachedBitcoinData = localStorage.getItem('bitcoinChartData');
    const cachedTimestamp = localStorage.getItem('bitcoinChartDataTimestamp');
    
    if (cachedBitcoinData && cachedTimestamp && (Date.now() - parseInt(cachedTimestamp) < 3600000)) {
        console.log("Using cached Bitcoin chart data");
        createBitcoinChart(JSON.parse(cachedBitcoinData));
        return;
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            localStorage.setItem('bitcoinChartData', JSON.stringify(data));
            localStorage.setItem('bitcoinChartDataTimestamp', Date.now().toString());
...
function createBitcoinChart(data) {
    const prices = data.prices.map(price => ({
        x: new Date(price[0]),
        y: price[1]
    }));

    bitcoinChart = new Chart(document.getElementById('bitcoinChart'), {
        type: 'line',
        data: {
            datasets: [{
                label: 'Bitcoin Price',
        ...
        success: function(data) {
            ...
            if (typeof data === 'object' && data !== null) {
                selectedCoins.forEach((coin, index) => {
                    ...
                    if (data[upperSymbol] && data[upperSymbol].USD !== undefined) {
                        if (liveChart.data.datasets[index]) {
                            liveChart.data.datasets[index].data.push({
                                x: time,
                                y: data[upperSymbol].USD
                            });
                        } else {
                            console.warn(`Dataset for ${upperSymbol} not found in chart`);
                        }
                    } else {
                        console.warn(`No data for ${upperSymbol}`);
                    }
                });
    ...
    pageTransition(() => {
        $('#mainContent').empty().append(`
            <div class="container about-content">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <img src="images/profile.jpg" alt="Elad Ya'akobovitch" class="img-fluid rounded-circle mb-3 profile-image">
                    </div>
                    <div class="col-md-8">
                        <h3>Developer</h3>
                        ...
                        <h5>Features</h5>
                        <ul>
                            <li>Coins Page: Displays a list of the top cryptocurrencies by market cap.</li>
                            <li>Reports Page: Shows a chart of the current prices of selected cryptocurrencies and Bitcoin's 30-day price history.</li>
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                $('.profile-image').addClass('show');
            }, 200);
            $('.about-content > *').each((index, element) => {
                setTimeout(() => {
...
$(document).on('click', '.remove-from-report', function() {
    ...
    saveSelectedCoins();
    $(this).closest('.coin-card').fadeOut(400, function() {
        $(this).remove();
        displaySelectedCoins(); // Refresh the display
        initLiveReport();
    });
});
...
function fetchMoreInfo(coinId, infoContent) {
    ...
    // Check cache first
    if (coinCache[coinId] && Date.now() - coinCache[coinId].timestamp < 3600000) {
...
function showRemoveCoinModal(newCoinId, newCoinSymbol) {
    ...
    let html = `
        ...
        <div class="coin-management">
            ${selectedCoins.map(coin => {
                const coinInfo = coinCache[coin.id] || {};
                return `
                    <div class="coin-card-modal" data-id="${coin.id}">
                        <img src="${coinInfo.image || 'images/coin-placeholder.png'}" alt="${coin.symbol}">
                        <span class="coin-name">${coin.symbol.toUpperCase()}</span>
                        <button class="remove-coin-btn" data-id="${coin.id}" data-symbol="${coin.symbol}">
    ...
    $(document).on('click', '.remove-coin-btn', function() {
        ...
        $(this).closest('.coin-card-modal').remove();
        updateToggles();
        saveSelectedCoins();
        if (selectedCoins.length < 5) {
